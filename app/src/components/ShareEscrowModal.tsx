      ...
      <div className="neu-card w-full max-w-md p-6 rounded-3xl space-y-6 relative border-2 border-indigo-500/30 shadow-2xl bg-[#e4ebf5] dark:bg-[#0d111a]">
        ...
        <div className="w-full">
          <input
            ...
            readOnly
            value={shareUrl}
            className="w-full px-4 py-3 neu-inset rounded-2xl text-xs font-mono text-slate-700 dark:text-slate-200 focus:outline-none border border-slate-300/40 dark:border-slate-800/80 truncate shadow-inner select-all"
          />
        </div>

        {/* 2x2 Action Buttons Grid */}
        <div className="grid grid-cols-2 gap-3">
          {/* Row 1, Col 1: Copy link */}
          <button
            type="button"
            onClick={handleCopy}
            className="w-full neu-card hover:bg-slate-200/60 dark:hover:bg-slate-800/60 text-slate-800 dark:text-slate-100 font-bold py-3 px-4 rounded-2xl text-xs flex items-center justify-center gap-2 border border-slate-300/50 dark:border-slate-800/60 transition-all cursor-pointer shadow-sm active:scale-98"
          >
            {copied ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4 text-slate-700 dark:text-slate-300" />}
        ...
        {/* QR Code SVG Display */}
        <div className="neu-inset p-4 rounded-2xl flex flex-col items-center justify-center bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-2">
